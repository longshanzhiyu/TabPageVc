...
open class TabPageViewController: UIPageViewController {
    ...
    open var selectedIndex: Int = 0 {
        didSet {
            tabView.updateCurrentIndex(selectedIndex, shouldScroll: true)
            displayControllerWithIndex(selectedIndex, direction: .forward, animated: true)
        }
    }
...
extension TabPageViewController: UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if scrollOffsetX < 0, beforeIndex == 0, isInfinity == false {
            scrollView.setContentOffset(CGPoint(x: 0, y: 0), animated: false)
        }
        else if scrollOffsetX > 0, beforeIndex == (tabItemsCount - 1), isInfinity == false {
            scrollView.setContentOffset(CGPoint(x: defaultContentOffsetX * CGFloat(beforeIndex), y: 0), animated: false)
        }
