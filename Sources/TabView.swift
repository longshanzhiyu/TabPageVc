...
internal class TabView: UIView {
    ...
    @IBOutlet fileprivate weak var fixCurrentBarView: UIView!
    
    @IBOutlet var fixCurrentBarWidthConstraint: NSLayoutConstraint!
    ...
    init(isInfinity: Bool, option: TabPageOption) {
       super.init(frame: CGRect.zero)
        ...
        fixCurrentBarView.backgroundColor = option.currentColor
        fixCurrentBarWidthConstraint.constant = option.currentBarWidth
        currentBarView.backgroundColor = option.currentBarWidth > 0.0 ? .clear : option.currentColor
        fixCurrentBarView.backgroundColor = option.currentBarWidth > 0.0 ? option.currentColor : .clear
        fixCurrentBarView.layer.cornerRadius = option.currentBarWidth > 0.0 ? option.currentBarHeight * 0.5 : 0
    ...
    func scrollCurrentBarView(_ index: Int, contentOffsetX: CGFloat) {
        ...
        if let currentCell = collectionView.cellForItem(at: currentIndexPath) as? TabCollectionCell, let nextCell = collectionView.cellForItem(at: nextIndexPath) as? TabCollectionCell {
            ...
            if isInfinity {
                ...
                if scrollRate > 0 {
                    if currentIndex == pageTabItemsCount - 1 {
                        currentBarViewLeftConstraint?.constant = currentCell.frame.minX
                    }
                    else {
